...
if __name__ == '__main__':
    N=3
    datafilename = [f'data_split/data14/data_user_{i}.csv' for i in range(1, N + 1)]
    parts = [part(n+1,datafilename[n],savefile='logs_FL_HE',dataset='14') for n in range(N)]                          
    for n in range(N):
...
# FL_HE
    ...
    for i in range(26):     
        ...
        model_paths = [f'./logs_FL_HE/14/part{j}/14exp{counter}/final{i+1}.pth' for j in range(1, N+1)]
        print('进行-第%d轮' % (i+1))
        # 使用加密器聚合和平均模型参数
        ...
        # 指定解密后模型的保存路径，并保存
        print('解密后模型并保存ing')
        save_path = f"./logs_FL_HE/14/pth/model_14exp{counter}_{i+1}.pth"
